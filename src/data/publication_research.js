export const PUBLICATION_INFO = [
  {
    id: 1,
    title: "Machine Learning for Breast Cancer Classification: A Comparative Study of Grid Search-Optimized SVM, Random Forest, and XGBoost.",
    authors: "Md. Mijanur Rahman, Sudip Sarker, Md Mahamadul Hasan Shaikat, Sourav Das, Md Rezaul Karim Khan.",
    venue: "2025 International Conference on Quantum Photonics, Artificial Intelligence, and Networking (QPAIN).",
    type: "Conference",   
    year: "29 September 2025",
    link: "https://ieeexplore.ieee.org/document/11172245"
  },
  // {
  //   id: 2,
  //   title: "Sustainable Energy Solutions in Urban Environments",
  //   authors: "Alice Johnson, Bob Lee",
  //   venue: "Environmental Science & Technology",
  //   type: "Journal",
  //   year: 2022,
  //   link: "https://example.com/paper2"
  // },
  // {
  //   id: 3,
  //   title: "Deep Learning Approaches for Image Recognition",
  //   authors: "Mary Adams, Tom Brown",
  //   venue: "Computer Vision Journal",
  //   type: "Journal",
  //   year: 2021,
  //   link: "https://example.com/paper3"
  // },
  // {
  //   id: 4,
  //   title: "Deep Learning, Deep Learning Approaches for Image Recognition",
  //   authors: "Mary Adams, Tom Brown",
  //   venue: "International Conference on Computer Vision",
  //   type: "Conference",  // <- conference
  //   year: 2021,
  //   link: "https://example.com/paper4"
  // },
  // {
  //   id: 5,
  //   title: "Next-Gen AI Applications in Healthcare",
  //   authors: "John Doe, Mary Adams",
  //   venue: "Healthcare AI Journal",
  //   type: "Journal",
  //   year: 2020,
  //   link: "https://example.com/paper5"
  // }
];
